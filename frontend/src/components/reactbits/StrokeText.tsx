...
const StrokeText = ({
  ...
  const gradId = `${wipeId}-gradient`;
  const effectiveStroke = strokeColor === 'gradient' && gradient ? `url(#${gradId})` : strokeColor;
  const effectiveFill = fillColor === 'gradient' && gradient ? `url(#${gradId})` : fillColor;

  return (
    <span
      ref={rootRef}
      className={`stroke-text ${trigger === 'hover' ? 'stroke-text--hover' : ''} ${className}`.trim()}
      style={{ ...style, '--stroke-text-height': `${Math.round(fontSize * 1.3)}px` } as any}
      role="img"
